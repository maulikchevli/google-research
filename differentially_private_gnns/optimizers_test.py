...
class DifferentiallyPrivateAggregateTest(parameterized.TestCase):
  ...
  def test_no_privacy(self):
    ...
    l2_norms_threshold = jax.tree_map(lambda p: jnp.finfo(jnp.float32).max,
                                      self.params)
    ...
    mean_grads = jax.tree_map(lambda g: g.sum(axis=0), self.per_eg_grads)
  ...
  def test_aggregated_updates_as_input_fails(self):
    ...
    l2_norms_threshold = jax.tree_map(lambda p: 0.1, self.params)
    dp_agg = optimizers.dp_aggregate(
        l2_norms_threshold=l2_norms_threshold,
        base_sensitivity=1.,
        ...
        init_rng=jax.random.PRNGKey(42))
    state = dp_agg.init(self.params)
    summed_grads = jax.tree_map(lambda g: g.sum(axis=0), self.per_eg_grads)
